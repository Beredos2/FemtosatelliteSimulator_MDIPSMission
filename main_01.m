...
%% Distributed measurement for fiexed time T=25000 in stereotaxic space S(cubic,10)
...
%Map = cell(max(index(1,:),max(index(2,:))));
...
xlabel("Epoch [s]")
xlim([1672531200 1.672558350500100e+09])
plot(myClock.timeline(:,2),mySwarm.network{1}{1}.D_m(:,1),'r')
plot(myClock.timeline(:,2),mySwarm.network{1}{1}.D_m(:,3),'b')
plot(myClock.timeline(:,2),mySwarm.network{1}{1}.D_m(:,2),'g')
yyaxis('right')
ylim([-7,7])
plot(myClock.timeline(:,1),theta_se,'k*')
legend('X_{ECI} component','Y_{ECI} component','Z_{ECI} component')
